...
function createMetaBlock(meta) {
  ...
  metaDiv.appendChild(document.createTextNode(
    "Modelo usado: " + (meta.model || "No reportado") + "\n" +
  ...
  if (meta.documents && meta.documents.length) {
    ...
    docs.className = "sources";

    const title = document.createElement("strong");
    title.textContent = "Documentos usados:";
    docs.appendChild(title);

    meta.documents.forEach((doc, index) => {
      const line = document.createElement("div");
      line.textContent = "[" + (index + 1) + "] " + doc.name + " · " + (doc.characters || 0) + " caracteres extraídos";
      docs.appendChild(line);
    });

    metaDiv.appendChild(docs);
  }
