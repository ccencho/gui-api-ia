...
app.post("/api/chat", upload.array("documents", 5), async (req, res) => {
  try {
    ...
    if (useWebSearch) {
      ...
      const searchData = await searchWeb(lastUserMessage.content, Number(maxResults || 5));
...
function normalizeBody(body) {
  return {
    ...
    maxResults: Number(parseMaybeJson(body.maxResults, 5) || 5)
  };
}
...
function buildMessages({ systemPrompt, messages, useWebSearch, webContext, documentContext }) {
  ...
  system +=
    ...
    "\n- Usa Markdown limpio y ordenado: títulos con ## o ###, negrita con **texto**, listas y tablas cuando ayuden a entender." +
    "\n- No muestres código Markdown crudo si puedes estructurar la respuesta de forma clara." +
