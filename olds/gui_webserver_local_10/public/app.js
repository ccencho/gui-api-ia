...
function renderCodeBlock(code, language = "code") {
  ...
  return (
    ...
    '<span>' + escapeHtml(cleanLanguage || "code") + '</span>' +
    '<button type="button" class="code-copy-btn">Copiar</button>' +
    '</div>' +
    '<pre><code class="language-' + escapeHtml(cleanLanguage) + '">' + highlighted + '</code></pre>' +
    '</div>'
  );
}
...
function highlightCode(code, language) {
  ...
  if (["html", "xml", "svg"].includes(language)) {
    html = html
      ...
      .replace(/(&lt;\/?[a-zA-Z][^&]*?&gt;)/g, match => {
        return match
          .replace(/(&lt;\/?[a-zA-Z0-9-]+)/, '<span class="code-token-tag">$1</span>')
          .replace(/([a-zA-Z_:][-a-zA-Z0-9_:.]*)(=)/g, '<span class="code-token-attr">$1</span>$2')
          .replace(/(&gt;)/g, '<span class="code-token-tag">$1</span>');
      })
  ...
  if (language === "css") {
    html = html
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="code-token-comment">$1</span>')
      .replace(/([.#]?[a-zA-Z_][a-zA-Z0-9_-]*)(\s*\{)/g, '<span class="code-token-selector">$1</span>$2')
      .replace(/([a-zA-Z-]+)(\s*:)/g, '<span class="code-token-property">$1</span>$2')
      .replace(/(#(?:[0-9a-fA-F]{3}){1,2}|\b\d+(?:px|rem|em|%|vh|vw)?\b)/g, '<span class="code-token-number">$1</span>');
    return html;
  }

  if (["javascript", "typescript", "json", "python", "bash", "powershell", "java", "cpp", "c", "csharp", "php", "go", "rust"].includes(language)) {
    html = html
      .replace(/(\/\/.*$|#.*$)/gm, '<span class="code-token-comment">$1</span>')
      .replace(/(&quot;.*?&quot;|'.*?')/g, '<span class="code-token-string">$1</span>')
      .replace(/\b(const|let|var|function|return|if|else|for|while|class|import|from|export|async|await|try|catch|throw|new|true|false|null|undefined|def|print|echo|public|private|static|void|int|string|float|double|bool)\b/g, '<span class="code-token-keyword">$1</span>')
  ...
  try {
    ...
    chat.messages.push({
      ...
      meta: {
        ...
        resolvedWebResults: data.resolvedWebResults,
